...
Reliance on external services is becoming more prevalent in our engineering ecosystem. As a result, we need cost-effective, secure, and reliable ways to test these services.

In this post, we'll examine some strategies and available tools for testing apps that use third-party integrations.

## Why you should test third-party integrations
...
There are a few different options for how to test these external services. Let’s start with the... less desirable options.
...
We’ve loved and used this approach ourselves (hell, we built [unmock](https://github.com/meeshkan/unmock-js)). There are still issues with this approach though. Namely, that you have to manually write all the tests - even if the input values are generated.
