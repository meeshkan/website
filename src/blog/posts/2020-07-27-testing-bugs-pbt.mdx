---
title: Finding and fixing testing bugs with fast-check
...
Recently, we published [Property-based testing for JavaScript developers](/blog/property-based-testing-javascript/). In it, we introduced an issue to show the limitations of example-based testing. Throughout the rest of the guide, we provided examples of how to use fast-check and other property-based testing principles.

But we didn't offer a fix to that initial bug.

We've been told that this is unsatisfying, so consider this post is our redemption.

💻 The tests featured in this post are available in this [corresponding GitHub repository](https://github.com/meeshkan/property-based-testing-for-js-devs/blob/main/tests/getNumberIntl.test.js).
...
Now imagine your website also operates in Germany. This means that the meaning of commas and decimals in numbers are switched (i.e. $1,200.99 in English would be $1.200,99 in German).
...
- It works with strings formatted using one of the accepted locales (in this case, that would be German).
...
This is a lot all at once, I know. So let's go through it step-by-step.
...
Prices only go to the second decimal place and we want to embody this in our test.
...
This method returns a string, but we need a number for our final assertion. So we'll wrap it in another built-in function, [`parseFloat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/parseFloat), and call the variable `fixedFloat`.

```javascript
const fixedFloat = parseFloat(testFloat.toFixed(2))
```

Finally, we'll pass two arguments to the `property` callback: `testFloat` (representing each generated floating-point number) and `locale` (representing the generated locale from `constantFrom`).

Altogether, our `property` function will look like this:
