...
description: I'll show how to recreate the OP Bank sandbox and enhance it using Meeshkan.
...
There are many ways to obtain API recordings using Meeshkan. It has connectors to [Kong API Gateway](https://konghq.com/kong/), a tool to convert tcpdump logs and a standalone reverse proxy. We'll use the reverse proxy. 
...
To make recordings using the reverse proxy, we need to first start Meeshkan in a recording mode:
```bash
meeshkan record
```
...
```bash
meeshkan build --input-file ./logs/sandbox.apis.op-palvelut.fi-recordings.jsonl  --mode mixed
```

This should create a file in the generated `specs` directory called `openapi.json`, which contains our spec. The `--mode` argument tells Meeshkan to create an OpenAPI spec in the `mixed` mode during recording. The `mixed` mode means that a mock may return either recorded or generated data - depending on whether or not it could find a match for a request. 

Now we have to change the target host to a Meeshkan proxy host. By default, this is `http://localhost:8000` followed by the target host in a path string. 
...
So now we can restart Meeshkan in mock mode:
```bash
...
Meeshkan allows us to define custom callbacks that can flexibly modify the response data, while also having access to a request data and internal key-value storage. Internal key-value storage allows us to maintain a state across a sequence of requests. To clean it, we need to call `DEL /admin/storage` in the Meeshkan admin server. This admin server is launched automatically with the mock server. 

Callbacks are Python functions decorated by the `@callback` decorator. Meeshkan loads them automatically on start from any `*.py` file located under the `./callbacks` directory. 
...
## What's next?
The mock we made covers the basic features of the OP Bank API. However, a lot of developers probably want to be able to mock validation errors. We're going to explain how to manage error responses with Meeshkan in a future tutorial. 


We would appreciate any questions as well as feature requests. Please, leave a comment or [create an issue on GitHub](https://github.com/meeshkan/meeshkan) if you have something to tell us. You may also follow us on [Twitter](https://twitter.com/meeshkanml) to stay in touch. Any feedback means a lot for us since we're developing a new and emerging tool.