...
slug: typedict-vs-dataclasses-in-python
date: 2020-03-27
authors: ["mike"]
published: true
tags:
...
You may have guessed by now, but generally, we prefer duck typing over classes. For this reason, we're very enthusiastic about Python's `TypedDict`. That said, in our Python project [hmt](https://github.com/meeshkan/hmt), we migrated from `TypedDict` to `dataclasses` for several reasons. Throughout the rest of this article, we'll explain why we made the move.
...
The above `to_person` will raise an error, whereas the `TypedDict` version won't. This means that, when an error arises, it will happen later down the line. Due to the way Python stack traces work, these types of errors are much harder to debug.
...
I should mention that if you're a fan of typing, you've found a happy home on the Internet where we have _tons_ of articles about types in lots of languages. Check out our [blog](https://meeshkan.com/blog). If you're working on an API and using any typing strategy, like OpenAPI or GraphQL, you _definitely_ should sign up for the Meeshkan beta. Meeshkan automatically makes sure your app lives up to its typed contracts 💪.
...
I should also mention that a lot of our Python-based stack is open-sourced on [github](https://github.com/meeshkan), and we also contribute regularly to [PureScript python](http://github.com/purescript-python). If you're hardcore into Python+typing, you should check that project out!
