...
We may send you _only_ this in the report _even though_ your app is crashing in other places. This gets back to the rehearsal idea from above - our goal is not to taunt you with the 1,000 ways we made your service crash, but to give you a few useful pieces of information that will actually be helpful.

Of course, the danger of giving an example like PubSub is that it excludes others. We also look at CRUD operations, calls to third-party APIs, field names, and hundreds of other indicators that may trigger a report.

I've seen a lot of companies resist bringing in a QA professional because they're still experimenting with a PubSub (or whatever) implementation and they're not sure if it's worth testing yet. I regularly hear folks say "Why bother bringing in QA if we may change the whole thing in two weeks anyway?" Meeshkan exists for exactly this category of ideas. From the day you add Postgres or PubSub or OAuth2 to a service, our algorithms know how to test these things in the most useful way.

## How much does this all cost?

My rule of thumb is to plan one full-time salary going into some form of QA for every ~6 engineering hires. The math works like this. Three people can generally develop a robust MVP or feature in a month. As this core team progresses, you want one person observing the development from the vantage point of the user (or machine) that will ultimately interact with the service, reporting back so that building stays on track. It will save approximately 1 week of work for the team after launching the feature.

One week of work for two teams of three equals six weeks of work saved, and remember, you've set aside a month's salary for this. So your save exceeds your spend by a factor of 1.5. Of course, this is just a rule of thumb, and it needs to scale up and down with your development velocity. One of the benefits of automated QA is that the less you have going on, the less you pay, and if there's nothing worth testing then we don't charge.
...
Create a free account or subscribe to our newsletter below. You won't regret it!
