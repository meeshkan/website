...
Now that that's done, we can create a specialized function called `addTwoHalves` that _only_ accepts values that are `1/2` and, consequentially, returns 1.
...
The amazing thing here is that, when you give the function a signature, the return value _has_ to be specialized to `IsOne`. Otherwise, it won't compile. That means that, by following the types, the PureScript compiler is smart enough to follow two constrained values through the insides of our addition function and know what constrained is supposed to come out the other side.
