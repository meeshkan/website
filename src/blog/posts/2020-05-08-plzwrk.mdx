...
In the lambda function `#c{(\e s -> return $ (consoleLogS browser) "clicked")}#`, we see two arguments passed to the listener: `e` is an event and `s` is the state. Speaking of state, let's see how `plzwrk` handles state!
