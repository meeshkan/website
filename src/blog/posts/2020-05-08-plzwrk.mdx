---
title: plzwrk - A Haskell front-end framework
description: Build type-safe, blazingly fast webapps using plzwrk, Asterius and Haskell.
slug: plzwrk
date: 2020-03-27
authors: ["mike"]
published: true
...
I really like programming in Haskell. When I learned about [Asterius](https://github.com/tweag/asterius), a Haskell-to-WebAssembly compiler, I thought it'd be fun to create a small frontend framework inspired by JavaScript frameworks like [Vue](https://vuejs.org/).

Voilà [plzwrk](https://github.com/meeshkan/plzwrk). I've used it to build a couple [toy websites](https://github.com/meeshkan/plzwrk) and I must say the process has been quite delightful! In this article, I'd like to present a brief overview of plzwrk. If you're a frontend dev, I hope you give it a try!
...
On the first line, we are creating a browser. `plzwrk` is designed so that it can work with any representation of a browser using the [`Browserful`](https://github.com/meeshkan/plzwrk/blob/master/src/Web/Framework/Plzwrk/Browserful.hs) type. `plzwrk` ships with two browser representations - one backed by [Asterius](https://github.com/meeshkan/plzwrk/blob/6ccee37d5190873d455b4d648d78dfda9ad59edc/src/Web/Framework/Plzwrk/Asterius.hs#L34) (which we use above) - and a [mock](https://github.com/meeshkan/plzwrk/blob/6ccee37d5190873d455b4d648d78dfda9ad59edc/src/Web/Framework/Plzwrk/MockJSVal.hs#L648) one that is useful for testing.

On the second line, we create our element to be inserted into the DOM. `plzwrk` has two ways to input the dom:
