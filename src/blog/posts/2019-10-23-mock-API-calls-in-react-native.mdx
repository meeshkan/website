...
slug: mock-api-calls-react-native
date: 2019-10-23
authors: ["kimmo"]
published: true
tags:
...
There are different ways to mock network calls from React Native. You could use dependency injection to inject a "fetching service" into the components. In React Native tests, you would replace the real service with a mock. Or you could use React's [Context](https://reactjs.org/docs/context.html) to wrap components in a "fetching service" context. Both of these solutions can work, but there should be a simpler way.

In this post, we are going to build a basic React Native app tested in end-to-end fashion. We use [Unmock](https://www.unmock.io) to serve mock data to the app. Unmock is an HTTP testing library using [node-mitm](https://github.com/moll/node-mitm) behind the scenes to intercept HTTP traffic. At the time of interception, it generates random data mocking the API.

We'll run our tests in Node.js with [Jest](https://jestjs.io). We use the[React Native Testing Library](https://github.com/callstack/react-native-testing-library) to render the component and trigger React hooks. You can find the repository for this project [here](https://github.com/unmock/unmock-react-native-example). The repository also includes instructions for running the app.

## Tour of the sample app

The sample React Native app shows a random cat fact fetched from the [Cat Facts API](https://alexwohlbruck.github.io/cat-facts/). The user can refresh the fact by pressing the button. The app in all its glory looks like this, running here in Android virtual device:

![Screenshot of the sample app](https://thepracticaldev.s3.amazonaws.com/i/7p5tawbc8at2u16d8l0v.png)

The code for the React Native app contains a single component defined in [App.tsx](https://github.com/unmock/unmock-react-native-example/blob/master/src/App.tsx). At a high-level, we define the `App` component like this:
...
The app component renders content based on the values of `loading` and `err`:
...
Now let's write some tests for our React Native app, remembering that the tests will execute on the computer. For on-device React Native mocking, check out [On-device Mocking of REST APIs in React Native](https://meeshkan.com/blog/on-device-mocking-of-rest-apis-in-react-native/).

### Prerequisites

The file [App.test.tsx](https://github.com/unmock/unmock-react-native-example/blob/master/__tests__/App.test.tsx) contains the tests. The first step in the tests is to fill in `fetch` (not available in Node.js) with [node-fetch](https://www.npmjs.com/package/node-fetch):
...
The first React Native test ensures that when the API returns a cat fact, the app contains the correct element:
...
The second test for success ensures that when user clicks the button, the app fetches a new fact from the API. We simulate button press with the `fireEvent` from `react-native-testing-library`:
...
To automatically test your apps interactions with APIs, we recommend signing up for the React Native beta on the Meeshkan Webapp.
