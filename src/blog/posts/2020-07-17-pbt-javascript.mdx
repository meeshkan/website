...
🐍 Prefer Python? A similar guide is available to help you learn [property-based testing in Python](/blog/property-based-testing-python/) instead.
...
There are many situations where an example-based test like this would work well and be enough to cover what you need. But there can be downsides. When you have to create every test case yourself, you're only able to test as many cases as you're willing to write. The less you write, the more likely it is that your tests will miss catching bugs in your code.
...
Issues like the one faced with the input type also helps you write your property-based tests because then it's more clear what the property you're testing for actually is.
...
By default, the property check will be run against 100 generated inputs. For many arbitraries, you can also [set a minimum or a maximum number of generated inputs](https://dubzzz.github.io/fast-check/#/1-Guides/Arbitraries). At first, running hundreds of test cases might feel excessive - but these numbers are reasonable (and even considered low) in the property-based testing realm.

Going back to the example test, if you add a `console.log(testFloat)` statement, you can peek at the _100 different generated input values_:
...
> Note: Your numbers will likely be different - and that's ok. You also might not want to log 100 random numbers in your terminal - and that's ok too. You can take our word for it.
