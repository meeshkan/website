---
title: Comparing Prism, Hoverfly, and Mem - Part 1
description: An article comparing three mocking frameworks
slug: comparing-prism-hoverfly-and-mem-part-1
authors: ["mike"]
date: 2020-02-18
tags:
  - api
...
_This is the first article of a two-part series comparing Prism, Hoverfly, and Mem. This article presents a high-level comparison. The [second one](https://meeshkan.com/blog/comparing-prism-hoverfly-and-mem-part-2/) shows they can be used with a specific API._
...
At Mem, we reverse engineer software. Our mission is to build a tool that can clone even the most complex systems with a high degree of accuracy.

We have recently begun to open-source more of our stack as we get closer to achieving our mission. In doing so, we've come up against some strong incumbents such as Prism and Hoverfly. In this article, I'd like to present both of those tools to our new open-source project [Mem](https://github.com/meeshkan/mem).
...
If Hoverfly and Prism had a baby, it would be Mem. Mem allows developers to build a mock server from recordings and OpenAPI specs. It exposes a Python API for building a persistance layer behind mocks. We also use it to tie together ML models to simulate realistic server traffic.

Mem ingests recordings in the [`http-types`](https://github.com/meeshkan/http-types) format. You can build these Mem recordings using middleware or a proxy. For example, let's say that we have an `express` server and want to do our recording from that. We can use the [Mem `express` middleware](https://github.com/meeshkan/express-middleware) to save recordings.
...
It's also possible to use Mem as a reverse proxy for recording.

```bash
$ mem record --log_dir ./logs
$ curl http://localhost:8000/http/echo.jsontest.com/a/b
```

Then, we use `mem build` to turn these recordings into an OpenAPI spec. In the example below, we mix the recordings with a preexisting spec. The result is an enhanced OpenAPI spec.

```bash
$ mem build --mode replay -i recording.json -a spec.yml -o enhanced_spec
```

If you use `gen` mode, then Mem will infer a spec from recordings instead of serving them rote.

```bash
$ mem build --mode gen -i recording.json -a spec.yml -o enhanced_spec
```

Like Hoverfly and Prism, Mem can be started in server mode.

```bash
$ mem mock -i enhanced_spec/
```

**Conclusion:** Mem blends together recordings and OpenAPI specs into an enhanced OpenAPI spec. We'd love to hear your feedback on our [GitHub issues page](https://github.com/meeshkan/mem/issues).
