...
Wouldn't it be nice if we could just ignore the log? We'd need to beef up our profunctor with a bit more muscle so it can pass through the log in addition to the payload. In short, we need to make our server _stronger_.
