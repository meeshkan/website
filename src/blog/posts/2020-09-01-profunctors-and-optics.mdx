...
Our `((+) 1.0)` server did its magic without ever knowing the inner workings of your algorithm. The "passwords" here were `green`, `blue` and `red`, that, mixed with our `((+) 1.0)`, produced a beautiful new pixel!
...
- The **Function** profunctor, that accepts `a`'s and "forgets them" in a recepticle called `r`, ignoring the output `b` (`newtype Forget r a b = Forget (a -> r)`). A forget that maps `a` to itself is called a `Getter`. The general operation is called a `Fold`.
