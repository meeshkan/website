...
You may be wondering, "Why are there two names for that thing profunctors do - `dimap` and `iso`?" The reason is because the profunctor of implementation of optics came about _after_ optics were created other ways. The genius of the profunctor implementation is that one of the classic optics - Iso - is the `dimap` function.
