...
Profunctors are a _really useful abstraction_. It took me a while to understand what they are, but once I did, I found myself using them in a lot of different situations, from digital signal processing to building servers to traversing large data structures.

In this article, I'd like to share the intuition I've built up about profunctors. The way I built up this intuition is by studying how profunctors can be used to create [optics](https://medium.com/@gcanti/introduction-to-optics-lenses-and-prisms-3230e73bfcfe#:~:text=Optics%20are%20a%20very%20useful,values%20in%20a%20data%20type.). By the end of the article, I hope you'll be motivated to use profunctors and optics in your day-to-day work!
...
Profunctors are not just functions, and this is where the abstraction kicks in. Profunctors are a _general way to conceive of IO_. This is why I find them so useful. As programmers, we deal with IO on a micro-level (ie chaining functions) and a macro-level (ie chaining audio plugins).
...
const modified = (a) => `${parseFloat(a + 1)}`
```
...
To see why, imagine that the boss comes along and she says "Hey, actually, our server needs to accept UTF8 byte strings and return byte strings." No prob, profunctors to the rescue!
