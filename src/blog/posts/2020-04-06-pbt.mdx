...
description: We'll teach you what property-based testing is and how to write your first test.
slug: property-based-testing-python
date: 2020-04-06
authors: ["fredi", "carolyn"]
published: true
twitterShareUrl: https://twitter.com/intent/tweet?text=A%20beginner%27s%20guide%20to%20property-based%20testing%20via%20%40meeshkan%20https%3A%2F%2Fmeeshkan.com%2Fblog%2Fproperty-based-testing-python%2F
linkedinShareUrl: https://www.linkedin.com/shareArticle?mini=true&url=https%3A%2F%2Fmeeshkan.com%2Fblog%2Fproperty-based-testing-python%2F&source=Meeshkan
tags:
  - testing
  - tutorial
  - beginners
...
Because this decoding scheme only works for non-negative integers, you need to specify the _generation strategy_ of your input data. That's why, in this example, `some.integers(min_value=0)` is added to the `@given` decorator. It restricts Hypothesis to only generate integers with a minimum value of zero.
...
Let's compare this with a slower solution that converts the integer to a binary string (using the [bin](https://docs.python.org/3/library/functions.html#bin) function in the Python standard library) and then counts the occurrences of the string `"1"` inside of it:
...
<Button
  as={Link}
  isExternal
  href="https://twitter.com/intent/tweet?text=@meeshkan%20I%20haven%27t%20used%20PBT%20because..."
  variantColor="red"
  fontWeight={900}
>
...
At [Meeshkan](https://meeshkan.com/), we're working to improve how people test their products. So no matter if you loved or loathed this guide, we want to hear from you. [Tweet at us](https://twitter.com/meeshkan) or [reach out on Gitter](https://gitter.im/Meeshkan/community) to let us know what you think.
