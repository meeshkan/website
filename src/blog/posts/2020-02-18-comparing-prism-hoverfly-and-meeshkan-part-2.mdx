---
title: Comparing Prism, Hoverfly, and Mem - Part 2
description: An article comparing three mocking frameworks
slug: comparing-prism-hoverfly-and-mem-part-2
authors: ["mike"]
date: 2020-02-18
tags:
  - api
...
_This is the second article of a two-part series comparing Prism, Hoverfly, and Mem. This article focuses on using all three tools to build a mock of the github API v3, whereas the [first article](https://meeshkan.com/blog/comparing-prism-hoverfly-and-mem-part-1/) focuses on a high level comparison of the tools._
...
_Team Meeshkan busy building Mem._

In the [previous article](https://meeshkan.com/blog/comparing-prism-hoverfly-and-mem-part-1/), we saw how to create a Prism server from an OpenAPI spec. We also saw how Hoverfly can play back recordings of server traffic. Lastly, we saw how Mem blends OpenAPI specs and recordings into a mock server.
...
The first step in creating a Mem mock is the same as the Prism mock - we download the OpenAPI spec.
...
Already here, we can start Mem in server mode an examine some results.

```bash
$ mem mock -a ./stripe-spec
...
- it allows Mem to pick up on features of the spec not present in the recordings and vice versa.
...
```bash
$ mem record --log_dir ./stripe-logs
...
```bash
$ mem build -i ./stripe-logs/ --mode gen -o build
$ mem mock -s build/
...
In this article, we saw how three mocking tools - Prism, Hoverfly and Mem - can create a mock of the Stripe API. I hope that, in going through this, you got a sense of the differences between the three tools. In short:

- Prism can create a mock of an OpenAPI spec
- Hoverfly can serve back recordings
- Mem can serve a mix of recordings and synthetic data
